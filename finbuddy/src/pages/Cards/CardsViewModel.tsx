...
import {
  filterTransactionsByCardId,
  mapToProcessedTransactions,
...
export const useCardsViewModel = () => {
  ...
  const {
    creditCardInvoices,
    isLoading: isLoadingCreditCardInvoices,
    fetchCreditCardInvoices,
  } = useCreditCardInvoiceStore();
  const { bankAccounts, fetchBankAccounts } = useBankAccountStore();
  const { banks } = useBanks();
  const { invoicesTransactions, fetchInvoiceTransactions } =
    useInvoiceTransactionsStore();
    ...
    if (creditCards.length > 0) {
      ...
      const fetchAllInvoices = async () => {
        const invoicePromises = creditCards.map((card) => {
          fetchCreditCardInvoices(card.id);
        });
  ...
  console.log("All transactions from all cards:", allTransactions);
  const processedTransactions = mapToProcessedTransactions(
    allTransactions,
    creditCardInvoices
  );
