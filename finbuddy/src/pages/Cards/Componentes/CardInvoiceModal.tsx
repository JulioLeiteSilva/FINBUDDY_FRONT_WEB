...
const CardDetailsModal: React.FC<CardDetailsModalProps> = ({
  ...
  useEffect(() => {
    if (card) {
      ...
      if (currentInvoice) {
        setSelectedMonth(
          new Date(currentInvoice.year, currentInvoice.month - 1)
        );
      }
    }
  }, [card]);
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions
      .filter(
        (tx) => tx.cardId === card.id && tx.invoiceId === currentInvoice.id
      )
      .sort((a, b) => b.date.getTime() - a.date.getTime());
  }, [transactions, card, currentInvoice]);

  const { invoiceTotalForMonth } = useMemo(() => {
    if (!card || !currentInvoice)
      return { isAfterClosingDate: false, invoiceTotalForMonth: 0 };
  ...
  return (
    <>
        ...
        <DialogContent dividers>
          ...
          {filteredTransactions.length > 0 ? (
            ...
            <Box sx={{ textAlign: "center", py: 5, color: "text.secondary" }}>
              <Typography>
                Nenhuma transação na fatura de{" "}
                {dayjs(selectedMonth).format("MMMM de YYYY")}.
              </Typography>
            </Box>
          )}
      ...
      <Dialog open={isConfirmingPayment} onClose={handleCloseConfirmDialog}>
        <DialogTitle>Confirmar Pagamento</DialogTitle>
        <DialogContent>
          <Typography>
            Você tem certeza que deseja pagar a fatura do cartão{" "}
            <strong>{formatCurrency(invoiceTotalForMonth)}</strong> para o
            cartão <strong>{card.cardName}</strong>?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseConfirmDialog} disabled={isPaying}>
            Cancelar
          </Button>
          <Button
